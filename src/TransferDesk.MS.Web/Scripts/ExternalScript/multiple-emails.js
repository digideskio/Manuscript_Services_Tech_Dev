(function( $ ){
	...
	$.fn.multiple_emails = function(options) {		
		...
		if (settings.theme.toLowerCase() == "Bootstrap".toLowerCase())
		{
		    ...
		    deleteIconHTML = '<a href="#" class="multiple_emails-close" title="Remove"><img title="Remove" src="@Url.Content("~/")/Images/DeleteRed.png" style="width:20px; height:20px;"></a>';
		}
		
		return this.each(function() {
			//$orig refers to the input HTML node
		    var $orig = $(this);		  
			var $list = $('<ul id="emailCollection" class="multiple_emails-ul" />'); // create html elements - list of email addresses as unordered list

			if ($(this).val() != '' && IsJsonString($(this).val())) {
				$.each(jQuery.parseJSON($(this).val()), function( index, val ) {
				    $list.append($('<li id="' + val.toLowerCase() + '" class="multiple_emails-email"><span class="email_name" data-email="' + val.toLowerCase() + '">' + val + '</span></li>')
			...
			*/
			function display_email(t, dupEmailCheck) {			   
				...
				for (var i = 0; i < arr.length; i++) {
				    VerifyEmailAddress(arr[i].toLowerCase());
					...
					if ( dupEmailCheck === true && $orig.val().indexOf(arr[i]) != -1 ) {
					    ...
					    $list.append($('<li id="' + arr[i].toLowerCase() + '" class="multiple_emails-email"><span class="email_name" data-email="' + arr[i].toLowerCase() + '">' + arr[i] + '</span></li>')
							  .prepend($(deleteIconHTML)
								   .click(function(e) { $(this).parent().remove(); refresh_emails(); e.preventDefault(); })
							  )
						);
				...
				if (errorEmails.length > 0) {
				    ShowFailureResponseMessage("Please enter a valid email address.");
